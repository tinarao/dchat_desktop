...
describe("key pairs", () => {
    ...
    test("keys in keypair should seamlessly convert to and from base64", () => {
        const keyPair = generateUserKeyPair();

        const publicBase64 = uint8ToBase64(keyPair.publicKey);
        expectTypeOf(publicBase64).toBeString();
        const publicDeserialized = base64ToUint8(publicBase64);
        expect(publicDeserialized).toStrictEqual(keyPair.publicKey);

        const privateBase64 = uint8ToBase64(keyPair.privateKey);
        expectTypeOf(privateBase64).toBeString();
        const privateDeserialized = base64ToUint8(privateBase64);
        expect(privateDeserialized).toStrictEqual(keyPair.privateKey);
    });
});
...
describe("room keys", () => {
    ...
    test("encrypted roomKey should be decrypted with their public key and be equal to itself", () => {
        ...
        const decryptedRoomKey = decryptRoomKeyForUser(
            encryptedRoomKey,
    ...
    test("group encryption should work for multiple participants", () => {
        ...
        const decryptedKey1 = decryptRoomKeyForUser(
            encryptedKeys[0],
            user1KeyPair.privateKey
        );
        expect(decryptedKey1).toEqual(roomKey);

        const decryptedKey2 = decryptRoomKeyForUser(
            encryptedKeys[1],
            user2KeyPair.privateKey
        );
        expect(decryptedKey2).toEqual(roomKey);

        const decryptedKey3 = decryptRoomKeyForUser(
            encryptedKeys[2],
            user3KeyPair.privateKey
        );
    ...
    test("should not decrypt with unknown key", () => {
        ...
        const roomKey = generateRoomKey();
        const myEncrRoomKey = encryptRoomKeyForUser(
            roomKey,
            myKeyPair.publicKey,
            myUserId.toString()
        );
        ...
        for (const _ in Array.from({ length: 500 }, (_, i) => i)) {
            expect(() => {
    ...
    test("adding participant should create new encrypted key", () => {
        ...
        const initialEncryptedKeys = createEncryptedRoomKeys(
            roomKey,
            initialParticipants
        );
        ...
        const updatedEncryptedKeys = addParticipantToRoom(
            roomKey,
            newParticipant,
            initialEncryptedKeys
        );
    ...
    test("removing participant should recreate keys for remaining users", () => {
        ...
        const updatedEncryptedKeys = removeParticipantFromRoom(
            roomKey,
            remainingParticipants
        );

        expect(updatedEncryptedKeys).toHaveLength(2);
        expect(updatedEncryptedKeys.every((key) => key.userId !== "user2")).toBe(
            true
        );
    });
    ...
    test("encryptedRoomKeyToBase64 should serialize key to base64 and deserialize it back", () => {
        ...
        const decr = decryptRoomKeyForUser(
            roomKeyFromBase64,
            keys.privateKey
        );
